...
func DisplaySuccessMessage(successMessage string) error {
	p := tea.NewProgram(model{successMessage: successMessage})
	if _, err := p.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		return err
	}
...
func (m model) View() string {
	if m.errorInterface != nil {
		...
		return SUCCESS_STYLE.Render(m.successMessage) + "\n"
	}
}
