	...
	for {
		...
		for _, c := range m.Captures {
			if c.Node.Type() == "package_identifier" && packageName == "" {
				...
				if annotatedRoute == "" {
					log.Printf("route details not set for %s\n", handlerName)
					continue
				} else {
					routeDetails, err := FromAnnotaion(annotatedRoute)
					if err != nil {
						log.Printf("bad annotated route Route Details - %v  Comment - %s \n", routeDetails, annotatedRoute)
						continue
					}
					//log.Printf("Annotated route Handler - %s -  Comment -  %s \n", handlerName, annotatedRoute)
